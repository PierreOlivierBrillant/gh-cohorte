...
// studentAssignment dit où une personne a déjà un dépôt.
...
// studentRow est une ligne de la liste des étudiants.
...
// handleStudents croise la liste des personnes avec l'inventaire : pour chacune,
// les travaux où un dépôt existe déjà. C'est l'équivalent du « a accepté le
// travail » de GitHub Classroom, déduit des dépôts plutôt que d'une invitation.
func (s *Server) handleStudents(writer http.ResponseWriter, request *http.Request) {
	org, err := valid.Login(request.PathValue("org"), "Organisation")
	if err != nil {
		fail(writer, err)
		return
	...
	built := make([]groups.Group, 0, len(detected))
	for _, item := range detected {
		built = append(built, groups.Build(item.Prefix, repos))
	}

	rows := make([]studentRow, 0, len(body.People))
	for _, person := range body.People {
		row := studentRow{
			FullName: person.FullName, Username: person.Username,
			Assignments: []studentAssignment{},
		}
		for _, group := range built {
			repo, _, found := group.Find(person.Username)
			if !found {
				continue
			}
			row.Assignments = append(row.Assignments, studentAssignment{
				Prefix: group.Prefix, Repo: repo.Name, URL: s.urlOf(org, repo),
			})
		}
		rows = append(rows, row)
	}
	writeJSON(writer, http.StatusOK, map[string]any{"students": rows})
}
